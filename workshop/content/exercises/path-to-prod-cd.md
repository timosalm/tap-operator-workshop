...
You can also see the first few lines of the Pod spec by clicking on the rectangle for the output next to the step in TAP-GUI. Not being able to see the full spec is currently a limitation of the Kubernetes max status size.

###### App Config

The App Config step **uses the Pod spec** generated by Cartographer Conventions **for the generation of the YAML for a deployment resource**. In the case of a **Workload of type web** this will be a **Knative Service** which you can see in TAP-GUI in the rectangle for the output of the step. More information on Knative will be provided later.
